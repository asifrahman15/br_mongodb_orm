...
from br_mongodb_orm import BaseModel, current_datetime
...
class User(BaseModel):
    ...
    async def get_posts(self) -> List['Post']:
        ...
        return await Post.filter(author_id=self.id).to_list()
...
class User(BaseModel):
    ...
    async def get_active_users(cls) -> List['User']:
        ...
        return await cls.filter(is_active=True).to_list()
...
class User(BaseModel):
    ...
    async def get_admins(cls) -> List['User']:
        """Get all admin users"""
        return await cls.filter(role=UserRole.ADMIN).to_list()
```
